          ...
          <scroll class="middle-r" ref="lyricList" :data="currentLyric && currentLyric.lines">
            <div class="lyric-wrapper">
              <div v-if="currentLyric">
                <p ref="lyricLine" class="text" :class="{'current': currentLineNum === i}" v-for="(line, i) in currentLyric.lines" :key="i">{{line.txt}}</p>
              </div>
            </div>
          </scroll>
        </div>
...
export default {
  ...
  methods: {
    ...
    handleLyric({lineNum, txt}) {
      ...
      if (lineNum > 5) {
        let lineEl = this.$refs.lyricLine[lineNum - 5] // 第5行歌词之后再移动，加深的歌词看起来就在中间
